...
# Import fixed visualization module
import visualizations_fixed as viz
...
def student_data_analysis(data):
    ...
    print("=" * 70)
    print("Visualizations are being saved as image files in 'visualization_outputs/' folder.")
    print("You can open these files to see the graphs.")
    
    viz.plot_correlation_heatmap(data)
    viz.plot_feature_vs_target(data)
...
def score_prediction_model(x_train, x_test, y_train, y_test, feature_names):
    ...
    print("=" * 70)
    print("Saving model comparison charts...")
    viz.plot_model_comparison(results)
    
    # Show feature importance for tree-based models
    for result in results:
        if result["name"] in ["Decision Tree", "Random Forest"]:
            print(f"Saving feature importance for {result['name']}...")
            viz.plot_feature_importance(result["model"], feature_names)
    
    # Show residual plots
    for result in results:
        print(f"Saving residual analysis for {result['name']}...")
        viz.plot_residuals(result["y_test"], result["predictions"], result["name"])
    
    print("\n✓ All model visualizations created successfully!")
    print("Check the 'visualization_outputs/' folder for image files.")
...
def conclusion(best_model):
    ...
    print("=" * 70)
    print("All visualizations have been saved in the 'visualization_outputs/' folder.")
    print("You can open these image files to view the graphs and charts.")


def main():
    print("Student Exam Score Prediction System with Visualizations")
    print("=" * 70)
    print("This version saves all graphs as image files for easy viewing.")
